...
home_page <- div(
      mainPanel(
          fluidRow(
              ...
              p("It is an important Geographic Information Science (GIS)-based technique that allows users to go beyond mapping to describe and visualise spatial distributions, identify spatial outliers and discover spatial patterns such as clusters or hot spots. This allows users to translate geospatial data into insights by examining the data with a quantitative lens to gain deeper understanding before proceeding on with the modeling."),
              p("The most common exploratory areas of the spatial data are as follows:"),
              p("- Distribution analysis"),
              p("- Outliers and Cluster analysis"),
              p("- Hot spot and Cold spot Areas analysis"),
              p("With the latest technological advancements in Unmanned Aerial Vehicles (UAV), remote sensing and the increasing use of GPS-enabled smartphones and devices, the demand for location-based analytics is increasing in every industry with many organisations and multidisciplinary researchers relying on Geospatial Analytics for making decisions and deriving solutions. Therefore, there is a significant demand for free and open-source spatial analysis research infrastructure to enable more users (researchers, students, industry practitioners, non-GIS users) to adopt geospatial analytics and find the value in 'where'."),
              p("GeoDa is one such software program built by Professor Luc Anselin and his team which allows users to perform an end-to-end spatial analysis exercise from data transformation, mapping and geovisualisation to ESDA, spatial autocorrelation analysis and finally spatial regression analysis. Therefore, with the objective of building a web-enabled geospatial tool using R statistics tool and packages, our project aims to use GeoDa as reference and translate it into a web-based R shiny application that is easily accessible and user-friendly for all.")
            ),
          
          
          fluidRow(
              h2("Project Objective"),
              p("1. Build a flexible, robust, user-friendly and web-enabled geospatial tool - GeoXplorer - a geospatial analysis tool created using R Shiny web application hosted on Shinyapp.io with the purpose of allowing users to conduct exploratory spatial data analysis on different use cases"),
              p("2. Conduct User Acceptance Testing (UAT) of the application by performing exploratory spatial data analysis on a chosen use case to demonstrate the useability of the built application and to identify bugs and areas of improvement"),
              p("3. Build a R Blog web page using R blogdown hosted on Netlify that showcases our project details and progress as well as three of the final deliverables - poster, final research paper and the application user guide")),
